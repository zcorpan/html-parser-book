...
In the typical case, the input for the HTML parser comes from the network. However, it can also come from script with the `document.write()` API, which complicates the model. This is discussed in the [`document.write()`](#documentwrite) section.
...
When seeing the `script` end tag, the tree builder executes the script. The details of how this works is… complicated. See the [`document.write()`](#documentwrite) section.
